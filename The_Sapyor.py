...
class MainWindow(QMainWindow):
    ...
    def expand_reveal(self, x, y):
        for _, _, cell in self.get_reveable_cells(x, y):
            cell.reveal()
    
    def get_reveable_cells(self, x, y, force=False):
    ...
    def update_timer(self):
        if self.status == STATUS_PLAY:
            n_secs = int(time.time()) - self._timer_start
    ...
    def button_pressed(self):
        if self.status == STATUS_PLAY:
            self.update_status(STATUS_FAILED)
            self.reveal_grid()
        elif self.status in (STATUS_FAILED, STATUS_SUCCES):
            self.update_status(STATUS_READY)
            self.reset()
    
    def handle_chord(self, x, y):
        to_reveal = []
        self.determine_to_save_reveal(x, y, to_reveal)
        for _, _, cell in to_reveal:
            cell.reveal()
    
    def determine_to_save_reveal(self, x, y, to_reveal):
        ...
        if flagged_count == base_cell.mines_around:
            for xi, yi, cell in self.get_reveable_cells(x, y, True):
                if (xi, yi) not in ((xq, yq) for xq, yq, _ in to_reveal):
                    to_reveal.append((xi, yi, cell))
                    self.determine_to_save_reveal(xi, yi, to_reveal)
